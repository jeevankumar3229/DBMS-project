...
$firstname1 = $lastname1 = $email1 = $password1 = $confirm_password1 =$color1= "";
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	if (empty(trim($_POST["email"]))) {
		...
		$sql = "SELECT EMAIL from register1 WHERE email = ?";
  ...
  if (empty(trim($_POST['color']))) {
	...
	if (empty($firstname1) && empty($lastname1) && empty($email1) && empty($password1) && empty($confirm_password1) && empty($color1) ) {
		$sql = " INSERT INTO register1 (FIRSTNAME, LASTNAME,EMAIL,COLOR,PASSWORD) VALUES (?, ?, ?,?, ?)";
		$stmt = mysqli_prepare($conn,$sql);
		if($stmt)
		{
...
<html lang="en" dir="ltr">
  ...
  <?php   echo $register;?>
    ...
    <div>
        <nav class="navbar">
            <ul>
                <li><a href="login1.php"><i class="fa fa-backward" aria-hidden="true"></i><b>Back</b></a></li>
            </ul>
        </nav>
    </div>
    <div class="center">
